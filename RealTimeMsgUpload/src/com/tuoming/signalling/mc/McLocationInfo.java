...
//开关
public class McLocationInfo extends SignallingCommon {
    ...
    public boolean init(String[] split, int subevtType) {
        try {
            if (subevtType == 196611) {
                serviceType = 0;
            } else if (subevtType == 196612) {
                serviceType = 1;
            }
            time = TypeChange.timeChange(split[McLocationIndex.time]);
            msisdn = split[McLocationIndex.msisdn];
            if (msisdn.length() >= 3) {
                msisdn = msisdn.startsWith("86") ? msisdn.substring(2) : msisdn;
            } else {
                msisdn = "";
            }
            imei = split[McLocationIndex.imei];
            imsi = split[McLocationIndex.imsi];
            rat = TypeChange.strToInteger(split[McLocationIndex.rat]);
            if (!(rat == 1 || rat == 2)) {
                rat = null;
            } else {
                if (rat == 1) {
            ...
            lacTac = TypeChange.strToInteger(TypeChange.getAfter5(split[McLocationIndex.lacTac]));
            ciEci = TypeChange.strToLong(TypeChange.getAfter5(split[McLocationIndex.ciEci]));
            gnbIp = "";
        } catch (Exception e) {
            e.printStackTrace();
            return false;
