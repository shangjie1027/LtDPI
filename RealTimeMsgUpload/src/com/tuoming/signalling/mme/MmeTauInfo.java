...
public class MmeTauInfo extends SignallingCommon {
    //3字节
    ...
    public boolean init(String[] split, Jedis jedis) {
        try {
            ...
            time = TypeChange.timeChange(split[MmeIndex.getMmeIndex(MmeTauIndex.time, n)]);
            msisdn = split[MmeIndex.getMmeIndex(MmeTauIndex.msisdn, n)];
            if (msisdn.length() >= 7) {
                msisdn = msisdn.startsWith("86") ? msisdn.substring(2) : msisdn;
            } else {
                msisdn = "";
            }
            imei = split[MmeIndex.getMmeIndex(MmeTauIndex.imei, n)];
            imsi = split[MmeIndex.getMmeIndex(MmeTauIndex.imsi, n)];
            rat = TypeChange.strToInteger(split[MmeIndex.getMmeIndex(MmeTauIndex.rat, n)]);
            ...
            gnbIp = split[MmeIndex.getMmeIndex(MmeTauIndex.gnbIp, n)];
        } catch (Exception e) {
            return false;
        }
        return true;
