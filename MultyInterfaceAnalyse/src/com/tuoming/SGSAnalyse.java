...
import com.tuoming.entity.sgs.SgsDecode;
import com.tuoming.entity.sgs.SgsIndex;
import com.tuoming.readfile.ReadFile;
import com.tuoming.readfile.SgsReadFile;
...
public class SGSAnalyse {
    public static void main(String[] args) {
        if (args.length < 11) {
            System.out.println("【Class    】:" + SGSAnalyse.class.getName());
            System.out.println("【Parameter】:网元表位置、时间粒度(minute)、大小粒度(k)、输入目录、备份目录、输出目录、redisIp、redisPwd、原始文件名时间索引（210000_1_LTE-S6a_20190610135320_00.csv)、排序缓冲（条）、排序超时时间（s）");
        ...
        ReadFile readFile = new SgsReadFile();
        //写文件Map网元->SgsWrite
        //每个网元对应一个写通道
        ...
        while (true) {
            ...
            while ((sort = readFile.list.getFirst()) != null) {
                sortOutTimeCount = 0;
                String[] split = sort.str.split(SgsIndex.splite, -1);
                CommonDecode line = new SgsDecode();
                line.decode(split);
                WriteUntil.dealData(line, publicTableMap, cycleTime, writeMap, outputPath, fileSize, WriteIndex.sgsWrite);
            }
