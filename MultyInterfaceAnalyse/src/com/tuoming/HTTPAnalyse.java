...
import com.tuoming.entity.s1uhttps.S1uhttpsIndex;
import com.tuoming.readfile.ReadFile;
import com.tuoming.readfile.HttpReadFile;
import com.tuoming.sort.SortEntity;
import com.tuoming.writefile.S1uhttpWrite;
...
public class HTTPAnalyse {
    ...
    public static void main(String args[]) throws IOException {
        ...
        //输入参数检查
        if(args.length <11){
            System.out.println("【Class    】:" + HTTPAnalyse.class.getName());
        ...
        ReadFile readFile = new HttpReadFile();
        //写文件Map网元->SgsWrite
        //每个网元对应一个写通道
        ...
        while (true){
            ...
            while ((sort = readFile.list.getFirst()) != null) {
                sortOutTimeCount = 0;
                String[] split = sort.str.split(S1uhttpDecode.splite, -1);
                CommonDecode line = null;
                switch(split.length)
                {
                    case 127:
                        //判断http https 做初始化   需要处理
                        line = new S1uhttpDecode();
                        line.decode(split);
                        break;
                    case 94:
                        line = new S1uhttpsDecode();
                        line.decode(split);
                        break;
                }
                WriteUntil.dealData(line, publicTableMap, cycleTime, writeMap, outputPath, fileSize, WriteIndex.httpWrite);
            }
