...
public class IupsRauInfo extends SignallingCommon {
    //3字节
    ...
    public boolean init(String[] split) {
        try {
            time=TypeChange.timeChange(split[IupsRauIndex.time]);
//            msisdn = split[IupsRauIndex.msisdn];
//            if (msisdn.length() >= 7) {
...
//            }
            msisdn = "";
            imei = split[IupsRauIndex.imei];
            imsi = split[IupsRauIndex.imsi];
            rat = TypeChange.strToInteger(split[IupsRauIndex.rat]);
            if (!(rat == 1 || rat == 2 || rat == 6)) {
                rat = null;
            }
            lacTac = TypeChange.strToInteger(split[IupsRauIndex.lacTac]);
            ciEci = TypeChange.strToLong(split[IupsRauIndex.ciEci]);
            serviceType = 5;
...
//                }
//            }
//            uenc = TypeChange.strToInteger(split[IupsRauIndex.uenc]);
            gnbIp = split[IupsRauIndex.gnbIp];
        } catch (Exception e) {
            return false;
        }
        return true;
