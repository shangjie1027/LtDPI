            ...
            for (String file : list) {
                ...
                while (fileBuffer.size() > 0) {
                    ...
                    if (!writeMap.keySet().contains(networkElement)) {
                        ...
                        Write coapWrite = new CoapWrite(time);
                        String fileName = coapWrite.getFile(cycleTime, networkElement);
                        ...
                        coapWrite.setWf(writeFile);
                        writeMap.put(networkElement, coapWrite);
                        //非第一条数据
                    } else {
                        Write coapWrite = writeMap.get(networkElement);
                        if (coapWrite.getWf() != null) {
                            if (coapWrite.getWf().write(line.toString())) {
                                coapWrite.getWf().close();
                                coapWrite.setWf(null);
                                FileDealUntil.renameFile(outputPath + "/" + coapWrite.getTmpName(), outputPath + "/" + coapWrite.getFinlName());
                            }
                        } else {
                            coapWrite.setFileSignCount(coapWrite.getFileSignCount() + 1);
                            coapWrite.setTime(time);
                            String file2 = coapWrite.getFile(cycleTime, networkElement);
