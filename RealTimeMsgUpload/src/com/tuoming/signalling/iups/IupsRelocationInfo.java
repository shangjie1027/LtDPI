...
public class IupsRelocationInfo extends SignallingCommon {
    //3字节
    ...
    public boolean init(String[] str, Jedis jedis) {
        return false;
    }

    @Override
    public boolean init(String[] split) {
        try {
            time = TypeChange.timeChange(split[IupsRelocationIndex.time]);
//            msisdn = split[IupsRelocationIndex.msisdn];
//            if (msisdn.length() >= 7) {
...
//            }
            msisdn = "";
            imei = split[IupsRelocationIndex.imei];
            imsi = split[IupsRelocationIndex.imsi];
            rat = TypeChange.strToInteger(split[IupsRelocationIndex.rat]);
            if (!(rat == 1 || rat == 2 || rat == 6)) {
                rat = null;
            }
            lacTac = TypeChange.strToInteger(split[IupsRelocationIndex.lacTac]);
            ciEci = TypeChange.strToLong(split[IupsRelocationIndex.ciEci]);
            serviceType = 5;
//            if (!msisdn.startsWith("460")) {
//                roamType = 4;
//            } else {
//                String substring = msisdn.substring(0, 7);
//                String flag1 = RedisUntil.get(jedis, substring);
//                if (flag1 == null) {
//                    roamType = 3;
//                } else if ("1".equals(flag1)) {
//                    roamType = 2;
//                } else if ("0".equals(flag1)) {
//                    roamType = 1;
//                }
//            }
//            uenc = TypeChange.strToInteger(split[IupsRelocationIndex.uenc]);
            gnbIp = split[IupsRelocationIndex.gnbIp];
        } catch (Exception e) {
            return false;
        }
        return true;
