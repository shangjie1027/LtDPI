...
public class McSwitchInfo extends SignallingCommon {
    //3字节
    ...
    public boolean init(String[] split, Jedis jedis) {
        try {
            time = TypeChange.timeChange(split[McSwitchIndex.time]);
            msisdn = split[McSwitchIndex.msisdn];
            if (msisdn.length() >= 7) {
                msisdn = msisdn.startsWith("86") ? msisdn.substring(2) : msisdn;
            } else {
                msisdn = "";
            }
            imei = split[McSwitchIndex.imei];
            imsi = split[McSwitchIndex.imsi];
            rat = TypeChange.strToInteger(split[McSwitchIndex.rat]);
            if (!(rat == 1 || rat == 2)) {
                rat = null;
            } else {
                if (rat == 1) {
            ...
            lacTac = TypeChange.strToInteger(TypeChange.getAfter5(split[McSwitchIndex.lacTac]));
            ciEci = TypeChange.strToLong(TypeChange.getAfter5(split[McSwitchIndex.ciEci]));
            serviceType = 5;
...
//            uenc = TypeChange.strToInteger(split[McSwitchIndex.uenc]);
            gnbIp = "";
        } catch (Exception e) {
            return false;
        }
